...
try:
    ...
    LONC = ds.variables['grid_center_lon'][:]
    LATC = ds.variables['grid_center_lat'][:]
    ds = Dataset(sys.argv[2],'r')
...
maskbdy = np.zeros((nz,ny,nx))
npnts = configbdy["npoints"]
frac = 1.0/float(npnts)
bdyval_n = np.linspace(frac,1.0,npnts)
bdyval_s = np.linspace(1.0,frac,npnts)
if configbdy["rbcs"]["north"]:
    for k in range(nz):
        for i in range(nx):
            maskbdy[k,-npnts:,i] = bdyval_n
if configbdy["rbcs"]["south"]:
    for k in range(nz):
        for i in range(nx):
            maskbdy[k,0:npnts,i] = bdyval_s
    ...
    for k in range(nz):
        for j in range(ny):
            ...
            if configbdy["rbcs"]["south"]:
                if j < npnts:
                   i1 = npnts-j
            maskbdy[k,j,nx-npnts+i1:nx-npnts+i2] = bdyval_n[i1:i2]
    ...
    for k in range(nz):
        for j in range(ny):
            ...
            if configbdy["rbcs"]["south"]:
                if j < npnts:
                   i2 = j
            maskbdy[k,j,i1:i2] = bdyval_s[i1:i2]
